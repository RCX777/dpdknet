...
def create_bridge(name: str, datapath_type: str = 'netdev', protocols: str = 'OpenFlow10') -> OvsBridge:
    bridge = OvsBridgeModel(name=name, datapath_type=datapath_type, protocols=protocols)
    return create_wrapper(bridge, OvsBridge)


def create_port[T: OvsPort](name: str, bridge_name: str, cls: type[T] = OvsPortVhostUser) -> T:
...
def create_flow(match: str, actions: str, bridge_name: str) -> OvsFlow:
    bridge = get_bridge_model_by_name(bridge_name)
    if not bridge:
        raise ValueError(f"Bridge '{bridge_name}' does not exist.")
