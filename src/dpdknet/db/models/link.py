...
class LinkModel(BaseModel):
    ...
    port_src: Mapped[OvsPortModel] = relationship(OvsPortModel, back_populates='links_src', foreign_keys=[port_src_id])

    port_dst_id: Mapped[int] = mapped_column(ForeignKey('ovs_ports.id'), nullable=False)
    port_dst: Mapped[OvsPortModel] = relationship(OvsPortModel, back_populates='links_dst', foreign_keys=[port_dst_id])

    flow_fwd_id: Mapped[int] = mapped_column(ForeignKey('ovs_flows.id'), nullable=True)
    flow_fwd: Mapped[OvsFlowModel] = relationship(OvsFlowModel, back_populates='links_src', foreign_keys=[flow_fwd_id])

    flow_bwd_id: Mapped[int] = mapped_column(ForeignKey('ovs_flows.id'), nullable=True)
    flow_bwd: Mapped[OvsFlowModel] = relationship(OvsFlowModel, back_populates='links_dst', foreign_keys=[flow_bwd_id])
