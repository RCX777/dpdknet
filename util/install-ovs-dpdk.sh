...
function ovs-dpdk {
    ...
    echo "Installing OVS-DPDK build dependencies..."
    apt update -y && apt install -y \
        build-essential \
        fakeroot \
        autoconf \
    ...
    echo "Downloading OVS-DPDK sources..."
    mkdir -p /usr/src
    cd /usr/src
    wget https://github.com/openvswitch/ovs/archive/refs/tags/v3.5.0.tar.gz
    tar xf v3.5.0.tar.gz
    export OVS_DIR=/usr/src/ovs-3.5.0
    cd $OVS_DIR

    echo "Building OVS-DPDK..."
    ./boot.sh
